...
object ToonRetryPolicy {
   ...
   * @return Schedule that logs retry attempts
   */
   ...
   * @param maxRetries Maximum number of retries
   * @param retryPredicate Predicate to determine if error should be retried
...
object ToonRetryOps {
  ...
  implicit class ToonEffectRetryOps[R, A](effect: ZIO[R, ToonError, A]) {
     ...
     * Retry with timeout.
     *
     * @param timeout Maximum time to spend retrying
     * @param timeoutError Error to fail with on timeout
     */
