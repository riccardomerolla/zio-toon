...
trait ToonStreamService {
   ...
   *
   * Each value is encoded independently and emitted as a complete TOON document.
   * Useful for processing large collections without loading everything into memory.
   *
   * @param values Stream of ToonValues to encode
   * @return Stream of encoded TOON strings
   */
   ...
   *
   * Elements are streamed and assembled into array format incrementally.
   * More efficient than collecting all elements first.
   *
   * @param elements Stream of array elements
   * @param key Optional key for the array
   * @return Stream of TOON string chunks forming the array
   */
   ...
   *
   * Each complete TOON document in the stream is decoded independently.
   * Errors are propagated in the error channel.
   *
   * @param input Stream of TOON strings to decode
   * @return Stream of decoded ToonValues
   */
   ...
   *
   * Useful for processing line-delimited TOON files or network streams.
   * Handles backpressure automatically.
   *
   * @param lines Stream of TOON document lines
   * @return Stream of decoded ToonValues
   */
   ...
   *
   * @param values Stream of ToonValues
   * @return Stream of round-tripped ToonValues
   */
  def roundTripStream(
...
object ToonStreamService {
  ...
  val live: ZLayer[ToonEncoderService & ToonDecoderService, Nothing, ToonStreamService] =
    ZLayer.fromFunction((encoder: ToonEncoderService, decoder: ToonDecoderService) =>
