 ...
 * @param savingsPercent The percentage of tokens saved (0-100)
 */
...
trait ToonJsonService {
   ...
   * @return A ZIO effect that produces the JSON string
   */
   ...
   * @return A ZIO effect that either fails with a string error or succeeds with ToonValue
   */
   ...
   * Calculate token savings between JSON and TOON formats.
   * 
   * This method encodes the value to both pretty-printed JSON and TOON formats,
   * then calculates the difference in character count and percentage savings.
   * 
   * @param value The ToonValue to analyze
   * @return A ZIO effect that produces TokenSavings statistics
   */
   ...
   * @return A ZIO effect that produces the pretty-printed JSON string
   */
