...
trait ToonSchemaService {
  ...
  /** Decode a TOON string to a value using its schema.
    ...
    * @return
    *   A ZIO effect that either fails with an error or succeeds with the value
    */
...
object ToonSchemaService {
  ...
  def configured(
    ...
    ZLayer.fromFunction((jsonService: ToonJsonService) => Live(encoderConfig, decoderConfig, jsonService))
}
