...
trait ToonEncoderService {
   ...
   * 
   * This is an effect that describes the encoding operation.
   * The actual encoding is deferred until the effect is executed.
   * 
   * Encoding never fails - it always produces a valid TOON string.
   * 
   * @param value The ToonValue to encode
   * @return A ZIO effect that produces the encoded string (infallible)
   */
...
object ToonEncoderService {
   ...
   * It uses default configuration (2-space indentation, comma delimiter).
   * 
   * @example
   * {{{
   ...
   * 
   * @param config Custom encoder configuration
   * @return A layer providing the configured encoder service
   * 
   * @example
   * {{{
   ...
   * from the ZIO environment and calls its encode method.
   * 
   * @param value The ToonValue to encode
   * @return A ZIO effect requiring ToonEncoderService that encodes the value
   * 
   * @example
   * {{{
   * // Direct use
