...
object ToonPropertySpec extends ZIOSpecDefault {
  ...
  /** Generate a safe key for TOON objects. Ensures the key is non-empty and contains only safe characters. Falls back
    * to a default key if filtering results in empty string.
    */
  val genSafeKey: Gen[Any, String] =
    Gen
      .alphaNumericStringBounded(3, 20) // Start with at least 3 chars
  ...
  /** Generator for simple objects (flat structure, no nesting). Keys are filtered to avoid TOON syntax conflicts.
    */
  ...
  /** Generator for tabular arrays (array of uniform objects). Field names are filtered to avoid TOON syntax conflicts.
    */
  ...
  /** Generator for nested TOON values (limited depth to avoid stack overflow). Keys are filtered to avoid TOON syntax
    * conflicts.
    */
