...
object ToonRetryPolicySpec extends ZIOSpecDefault {
  ...
  def failingDecoder(attempts: Ref[Int], failUntil: Int, error: ToonError)
      : ZIO[ToonDecoderService, ToonError, ToonValue] =
    for {
  ...
  def spec = suite("ToonRetryPolicy")(
    ...
    suite("Extension Methods")(
      ...
      test("retryWithTimeout fails after timeout") {
        val neverSucceeds = ZIO
          .fail(ToonError.ParseError("always fails"))
          .delay(100.millis)
